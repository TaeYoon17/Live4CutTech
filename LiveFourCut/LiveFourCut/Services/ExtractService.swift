...
class ExtractService{
    ...
    func extractFrameImages() async throws -> [[URL]] {
        ...
        return try await extractPararell()
    }
    ...
    func extractSerial() async throws -> [[URL]] {
...
//        print("totalImageDatas \(totalImageDatas.count)")
    ...
    func extractPararell() async throws -> [[URL]] {
        return try await withThrowingTaskGroup(of: (Int,[URL]).self) { taskGroup in
            for (offset,v) in avAssetContainers.enumerated(){
                taskGroup.addTask {[minDuration,fps] in
                    ...
                    var imageURLs:[URL] = []
                    lastImage = imgContain.image
                    let lastURL = self.saveFrameImage(lastImage, at: offset, for: 0)!
                    imageURLs.append(lastURL)
                    for idx in (1..<Int(minDuration * fps)){
                        let time = CMTime(seconds: Double(idx) / 24, preferredTimescale: 600)
                        let imgContain = try? await generator.image(at: time)
                        if let imgContain{
                            let url = self.saveFrameImage(imgContain.image, at: offset, for: idx)!
                            imageURLs.append(url)
                            lastImage = imgContain.image
                        }
                        else{
                            let url = self.saveFrameImage(lastImage, at: offset, for: idx)!
                            imageURLs.append(url)
                        }
                    }
                    return (offset,imageURLs)
                }
            }
            var imageContainers: [[URL]] = Array(repeating:[], count: frameCounts)
